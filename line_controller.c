...
static int debounce_on_color = 0;
...
static void better_stop() {
    motor_run(MOTOR_FL, 10, BACKWARD);
    motor_run(MOTOR_FR, 10, BACKWARD);
    motor_run(MOTOR_RL, 10, BACKWARD);
    motor_run(MOTOR_RR, 10, BACKWARD);
    debounce_on_color = 1;
}
...
void apply_line_pattern(int L, int M, int R, int *last_direction, const char *log_prefix) {
    ...
    if (strcmp(rgb.name, "Red") == 0) {
        ...
        if (!debounce_on_color) better_stop();
        motor_stop_all();
        return;
    }
